...
const Page: NextPage<{ userId: APIUser["id"] }> = ({ userId }) => {
  ...
  const isMe = currentUserQuery.isSuccess && userId === currentUserQuery.data.id

  useEffect(() => {
    if (isMe) {
      setNavigationTitle(currentUserQuery.data?.nickname)
    }
  }, [isMe])
  ...
  if (isMe && myProfileQuery.isSuccess) {
    ...
    return (
      <div>
        <MainInfoContainer>
          <MainInfoArea>
            <Avatar size="xl" src={profileImage ?? DEFAULT_PROFILE_IMAGE_URL} />
            <StatBar>
              <div>
