...
const middleware: NextMiddleware = async (request) => {
  ...
  if (isProxy) {
    ...
    if (slamToken) {
      headers.set("Authorization", `Bearer ${slamToken}`)
    }
  ...
  if (request.nextUrl.pathname.startsWith("/logout")) {
    ...
    const response = NextResponse.redirect(nextUrl)
    response.cookies.set(env.SLAM_TOKEN_KEY, undefined, {
  ...
  const isPrivatePath =
    !!PRIVATE_PATHS.find((path) => request.nextUrl.pathname.includes(path)) ||
