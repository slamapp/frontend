...
const queryFn = () => api.reservations.getMyUpcomingReservations().then(({ data }) => data)
...
export function useGetUpcomingReservationsQuery(options: { enabled: true }): ResultSuccess
export function useGetUpcomingReservationsQuery(options: { enabled: false }): UseSuspenseQueryResultOnLoading
export function useGetUpcomingReservationsQuery(options: {
  enabled: boolean
}): UseSuspenseQueryResultOnLoading | ResultSuccess
export function useGetUpcomingReservationsQuery(options?: { enabled?: boolean }) {
