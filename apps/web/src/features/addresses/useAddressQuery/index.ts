...
export const useAddressQuery = (position: Parameters<typeof key.addresses.byPosition>[0]) =>
  useSuspenseQuery(
    key.addresses.byPosition(position),
    () =>
      new Promise<string>((resolve) => {
        const latLng = new kakao.maps.LatLng(position.latitude, position.longitude)
        new kakao.maps.services.Geocoder().coord2RegionCode(latLng.getLng(), latLng.getLat(), (result, status) => {
